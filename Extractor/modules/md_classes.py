...
@app.on_message(filters.command(["md_classes"]) & filters.user(SUDO_USERS))
async def md_classes_account(_, message):
    global cancel
    cancel = False
    editable = await message.reply_text("Send **ID & Password** in this manner otherwise bot will not respond.\n\nSend like this:-  **ID*Password**")
  
    rwa_url = "https://mdclassesapi.classx.co.in/post/userLogin"
    ...
    hdr1 = {
        "Host": "mdclassesapi.classx.co.in",
        "Client-Service": "Appx",
        "Auth-Key": "appxapi",
        "User-Id": userid,
        "Authorization": token
    ...
    res1 = requests.get("https://mdclassesapi.classx.co.in/get/mycourse?userid="+userid, headers=hdr1)
    ...
    html = scraper.get("https://mdclassesapi.classx.co.in/get/course_by_id?id=" + raw_text2,headers=hdr1).json()
    course_title = html["data"][0]["course_name"]
    scraper = cloudscraper.create_scraper()
    html = scraper.get("https://mdclassesapi.classx.co.in/get/allsubjectfrmlivecourseclass?courseid=" + raw_text2,headers=hdr1).content
    ...
    try:
        ...
        for y in range(0,len(xv)):
            raw_text3 =xv[y]
            res3 = requests.get("https://mdclassesapi.classx.co.in/get/alltopicfrmlivecourseclass?courseid=" + raw_text2,"&subjectid=" + raw_text3, headers=hdr1)
            ...
            for data in b_data2:
                ...
                res4 = requests.get("https://mdclassesapi.classx.co.in/get/livecourseclassbycoursesubtopconceptapiv3?courseid=" + raw_text2 + "&subjectid=" + raw_text3 + "&topicid=" + tid + "&start=-1",headers=hdr11).json()
                topicid = res4["data"]
                print(topicid)
                for data in topicid:
                  tid = (data["Title"])
