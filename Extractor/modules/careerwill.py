...
async def careerdl(app, message, headers, raw_text2, name, prog):
    num_id = raw_text2.split('&')
...
async def career_will(_, message):
    ...
    try:
        thread = threading.Thread(target=lambda: asyncio.run(careerdl(app, message, headers, raw_text2, name, prog)))
        thread.start()
