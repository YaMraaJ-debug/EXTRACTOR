...
@app.on_message(filters.command(["e1_coaching"]) & filters.user(SUDO_USERS))
async def e1_coaching_account(_, message):
    global cancel
    cancel = False
    ...
    html = scraper.get("https://e1coachingcenterapi.classx.co.in/get/course_by_id?id=" + raw_text2,headers=hdr1).json()
    ...
    try:
        ...
        for y in range(0,len(xv)):
            ...
            for data in b_data2:
                ...
                res4 = requests.get("https://e1coachingcenterapi.classx.co.in/get/livecourseclassbycoursesubtopconceptapiv3?courseid=" + raw_text2 + "&subjectid=" + raw_text3 + "&topicid=" + tid + "&start=-1",headers=hdr11).json()
                topicid = res4["data"]
                print(topicid)
                for data in topicid:
                  tid = (data["Title"])
