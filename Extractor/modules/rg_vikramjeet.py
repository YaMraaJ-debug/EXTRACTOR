...
@app.on_message(filters.command(["rg_vikramjeet"]) & filters.user(SUDO_USERS))
async def rg_vikramjeet_account(_, message):
    global cancel
    cancel = False
    ...
    res1 = requests.get("https://rgvikramjeetapi.classx.co.in/get/mycourse?userid="+userid, headers=hdr1)
    ...
    try:
        ...
        for y in range(0,len(xv)):
            ...
            for data in b_data2:
                ...
                res4 = requests.get("https://rgvikramjeetapi.classx.co.in/get/livecourseclassbycoursesubtopconceptapiv3?courseid=" + raw_text2 + "&subjectid=" + raw_text3 + "&topicid=" + tid + "&start=-1",headers=hdr11).json()
                topicid = res4["data"]
                print(topicid)
                for data in topicid:
                  tid = (data["Title"])
